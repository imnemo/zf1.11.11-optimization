...
class Zend_Controller_Front
{
     ...
     *
     * 设置前端控制器的baseUrl变量，如果已经设置_request变量了，同时会设置$_request的baseUrl，然后在处理ini配置时，貌似还没有为$this->_request初始化，且
     * $this->getRequest()也没有自动去初始化request实例，因此ini里设置baseUrl目前只能设置前端控制器的baseUrl变量
     * 
     * @param string $base
     * @return Zend_Controller_Front
     ...
     * 如果设置了$this->_request则有些使用request实例的baseUrl，然后是本实例的
     *
     * @return string
     */
...
//             var_dump(Zend_Controller_Action_HelperBroker::getStack());
        }
         ...
         * 
         * 如果不设置resources.frontController.noErrorHandler = 1，那么默认注册的控制器插件就只有本方法开始注册的ErrorHandler一个
         * $this->plugins->getPlugins == array(200 => Object(Zend_Controller_Plugin_ErrorHandler))
         * 用来收集dispatch请求过程中的所有Exception，细节参见@link Zend_Controller_Plugin_ErrorHandler
         */
//         var_dump(array_keys($this->_plugins->getPlugins()));
        $this->_plugins
         ...
         * Initialize router
         * 
         * InvokeArgs也要传到router中去，说明在配置文件中也可以配置路由的一些参数
         */
//         var_dump($this->getParams());
         ...
         * 
         * 将InvokeArgs传给dispatcher，dispatcher会在dispatch时，将这些参数传给action controller类，在控制器中
         * 使用$this->getInvokeArg($key) 或者 $this->getInvokeArgs()获取
         */
        $dispatcher = $this->getDispatcher();
...
//                 var_dump($request->getParams(), $request->getModuleName(), $request->getControllerName(), $request->getActionName());
