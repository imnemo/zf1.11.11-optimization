...
//             $this->_routes = array('a'=>1) + $this->_routes;
//             var_dump($this->_routes);
			/**
			 * 好吧 array + array你赢了
			 * 经试验，确实可以这么做，而且还能加好多个array
			 * 相加过程中，下标无论是数字还是字符串，均以前面出现的array key为准，忽略后面相加array里重复的key
			 * array(1) + array(2) = array(1), 因为下标都是0, 后面的会被忽略
			 * array('a'=>1) + array('b'=>2, 'a'=>3) + array('c'=>4) = array('a'=>1, 'b'=>2, 'c'=>4) 
			 * 
			 * 把default route放到最前面，保证最后使用，因为Zend_Controller_Front::dispatch->self::route时
			 * 执行了一下array_reverse($this->_routes, true)，保证自定义的路由优先匹配
			 */
