...
//     	var_dump($_SERVER);
        if ($requestUri === null) {
            ...
            /**
             * 真贴心
             * 自定义requestUri的时候，考虑到了有可能从原来的uri里面提取一部分作为query的情况（如把路径中的语言信息提取出来）
             * 这时候，需要手动将他们设置到$_GET里面，以便从$_GET中或者$this->getParams()中获取
             */
...
//         var_dump($requestUri, $baseUrl, $baseUrlRaw, $this->_pathInfo);
        return $this;
    }
