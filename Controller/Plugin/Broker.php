...
class Zend_Controller_Plugin_Broker extends Zend_Controller_Plugin_Abstract
{
    ...
    public function registerPlugin(Zend_Controller_Plugin_Abstract $plugin, $stackIndex = null)
    {
        ...
        if ($stackIndex) {
        	...
        	/**
        	 * 可以看到，如果没有设置stackindex时，自动获取一个
        	 * 获取的方法是先获取已有插件个数，然后以此为index依次往后查找是否被占用，没有的话则可以使用
        	 * 因此并不是，未指定index的时候，默认设置一个最大的index排到最后，而有可能插到某个节点，因为此前设置的index是不均匀的
        	 * 如开始设置了一个index = 20，然后注册一个plugin不指定index，这时自动设置的index就成了1
        	 */
